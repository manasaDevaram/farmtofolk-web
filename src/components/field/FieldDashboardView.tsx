  ...
  return (
    <AdminShell>
      <PageHeader
        ...
        description="Here are your verification tasks and farm visits."
        eyebrow="Verification workspace"
        title="Verifier Dashboard"
      />

      {loading ? <LoadingState label="Loading verification tasks..." /> : null}
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <SummaryMetric
              ...
              tone="kraft"
              value={pending.filter((item) => item.status.toUpperCase().includes("APPROVED")).length}
            />
            <SummaryMetric
                  ...
                  {pending.map((item) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm text-[var(--ftf-muted)]">
                        {verificationLocation(item) || "Location unavailable"} · {officerName(item)}
                      </p>
                    </article>
                  ))}
                </div>
